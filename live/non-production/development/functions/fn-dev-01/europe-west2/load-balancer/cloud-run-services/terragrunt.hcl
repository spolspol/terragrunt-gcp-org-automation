...
dependency "cert_map" {
  ...
  mock_outputs = {
    certificate_map_id = "projects/mock-project-id/locations/global/certificateMaps/fn-dev-01-cloud-run-cert-map"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan", "init"]
  skip_outputs                            = false
}
...
dependency "cloud_armor" {
  ...
  mock_outputs = {
    policy_self_link = "projects/mock-project-id/global/securityPolicies/fn-dev-01-cloud-run-lb-policy"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan", "init"]
  skip_outputs                            = false
}
...
dependency "webhook_handler" {
  ...
  mock_outputs = {
    service_name = "fn-dev-01-webhook-handler"
    service_uri  = "https://fn-dev-01-webhook-handler-mock.run.app"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan", "init"]
  skip_outputs                            = false
}
...
dependency "api_service" {
  ...
  mock_outputs = {
    service_name = "fn-dev-01-api-service"
    service_uri  = "https://fn-dev-01-api-service-mock.run.app"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan", "init"]
  skip_outputs                            = false
}
