# Cloud SQL database password secret
...
locals {
  ...
  secret_value = get_env("TF_VAR_cr_db_password", "")
}
...
inputs = {
  ...
  secret_accessors_list = [
    ...
    "serviceAccount:${dependency.runner_sa.outputs.email}",
  ]
  labels = {
    "${local.secret_name}" = merge(
      include.secrets_common.locals.common_secret_labels,
      {
        ...
        purpose     = "cloud-sql-password"
        environment = include.base.locals.environment
        used_by     = "cloud-run-services"
      }
    )
  }
}
