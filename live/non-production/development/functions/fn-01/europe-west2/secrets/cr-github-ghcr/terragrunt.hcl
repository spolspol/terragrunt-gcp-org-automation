# GitHub Container Registry authentication secret
...
inputs = {
  ...
  secret_accessors_list = [
    ...
    "serviceAccount:service-${try(dependency.project.outputs.project_number, "000000000000")}@gcp-sa-artifactregistry.iam.gserviceaccount.com",
  ]
  labels = {
    "${local.secret_name}" = merge(
      include.base.locals.merged.common_secret_labels,
      {
        ...
        registry    = "ghcr-io"
        used_by     = "artifact-registry-remote"
      }
    )
  }
}
