...
inputs = merge(
  read_terragrunt_config("${get_repo_root()}/_common/templates/cloud_storage.hcl").inputs,
  {
    ...
    versioning = {
      ...
      lower(name) => lookup(
        local.merged_vars.environment_type == "production" ? { versioning_enabled = true } : { versioning_enabled = true },
        "versioning_enabled",
        true
      )
    }
    ...
    # Lifecycle rules for state management - keep state files for 1 year, then delete
    lifecycle_rules = [
      {
        action = {
          type = "Delete"
