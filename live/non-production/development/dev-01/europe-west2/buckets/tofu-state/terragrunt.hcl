...
inputs = merge(
  read_terragrunt_config("${get_repo_root()}/_common/templates/cloud_storage.hcl").inputs,
  {
    ...
    versioning = {
      ...
      lower(name) => true  # Always enable versioning for state buckets
    }
    force_destroy = {
      for name in ["${dependency.project.outputs.project_name}-${local.bucket_folder_name}"] :
      lower(name) => false  # Never allow force destroy for state buckets
    }
    bucket_policy_only = {
      for name in ["${dependency.project.outputs.project_name}-${local.bucket_folder_name}"] :
      lower(name) => true  # Enforce uniform bucket-level access
    }

    # Lifecycle rules for state management - keep old versions for compliance
    lifecycle_rules = [
      {
        action = {
          type = "Delete"
        ...
        condition = {
          ...
          num_newer_versions = 30   # Keep at least 30 recent versions
          with_state         = "ARCHIVED"
        }
      }
    ]
