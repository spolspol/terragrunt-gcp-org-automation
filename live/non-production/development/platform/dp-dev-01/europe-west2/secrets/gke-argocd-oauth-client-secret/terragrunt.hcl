# ArgoCD OAuth client secret for GKE cluster
...
locals {
  ...
  secret_value = get_env("TF_VAR_argocd_oauth_client_secret", "")
}
...
inputs = {
  ...
  labels = {
    "${local.secret_name}" = merge(
      ...
      {
        secret_type = "oauth-credential"
        purpose     = "argocd-authentication"
